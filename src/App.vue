<script setup lang="ts">
import { ref } from 'vue';

const skills = ref([
  'Excel', 'Word', 'Power Point',
  'Publisher', 'SILVACO Gateway', 'Mentographic',
  'Monte Carlo', 'SPICE', 'TCAD'
])

const showBorder = ref(false)
</script>

<template>
  <div
    class="page"
    :class="{ 'show-page-border': showBorder }"
  >
    <div class="flex">
      <div class="profile-pic bg-slate-500 rounded-xl w-[80%] m-4"></div>
      <div class="flex flex-col justify-start py-4">
        <span class="text-2xl font-bold">Nur Syafiqah Binti Yusop</span>
        <span class="text-base text-slate-500">QC Engineer</span>
        <span class="text-sm">Experienced QC engineer, technical support, and admin. Passionate, motivated, always eager to learn, adapt to new cultures, and embrace fresh challenges in the dynamic work environment.</span>
      </div>
    </div>
    <div
      class="bg-slate-200 min-w-[10in] relative left-[50%] flex justify-center items-center"
      style="transform: translateX(-50%);"
    >
      <div class="flex justify-around w-[7.25in] px-4 py-2">
        <div class="flex justify-start items-center my-1">
          <svg class="w-6 h-6 primary-text mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"></path></svg>
          <span class="text-sm">nursyafiqahyusop@gmail.com</span>
        </div>
        <div class="flex justify-start items-center my-1">
          <svg class="w-6 h-6 primary-text mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
          <span class="text-sm">Kuala Lumpur, Malaysia</span>
        </div>
        <div class="flex justify-start items-center my-1">
          <svg class="w-6 h-6 primary-text mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
          <span class="text-sm">60137426254</span>
        </div>
      </div>
    </div>
    <div class="flex flex-col justify-start items-start px-4 pt-4 mb-2">
      <span class="text-xl font-bold primary-text mb-2">Technical Skills</span>
      <div class="w-full flex flex-wrap justify-around">
        <span
          v-for="skill of skills"
          :key="skill"
          class="primary-bg text-sm font-semibold px-[20px] py-2 rounded mb-2 text-center h-fit"
        >{{ skill }}</span>
      </div>
    </div>
    <div class="flex flex-col justify-start items-start px-4 pt-4 mb-2">
      <span class="text-xl font-bold primary-text mb-2">Education</span>
      <ol class="grid grid-cols-2 gap-8 border-t-2 border-gray-200 mt-4">
        <li class="flex flex-col justify-start">
          <div class="relative bottom-3.5 flex items-center">
            <div class="flex z-10 justify-center items-center w-6 h-6 bg-blue-100 rounded-full shrink-0">
                <svg class="w-4 h-4 primary-text" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 14l9-5-9-5-9 5 9 5z"></path><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222"></path></svg>
            </div>
            <div class="hidden w-full border-t-2 border-gray-200 h-0"></div>
          </div>
          <div class="flex flex-col justify-start items-start">
            <span class="leading-5 text-base font-bold">Bachelor of Electronic Engineering</span>
            <span class="leading-5 text-sm font-light">Universiti Teknikal Malaysia, Melaka</span>
            <span class="leading-5 text-xs font-light">May, 2010 - Oct, 2014</span>
            <p class="leading-5 text-sm text-justify">Major in Wireless Communication</p>
          </div>
        </li>
        <li class="flex flex-col justify-start">
          <div class="relative bottom-3.5 flex items-center">
            <div class="flex z-10 justify-center items-center w-6 h-6 bg-blue-100 rounded-full shrink-0">
              <svg class="w-4 h-4 primary-text" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 14l9-5-9-5-9 5 9 5z"></path><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222"></path></svg>
            </div>
            <div class="hidden w-full border-t-2 border-gray-200 h-0"></div>
          </div>
          <div class="flex flex-col justify-start items-start">
            <span class="leading-5 text-base font-bold">MSc in Electronic Engineering</span>
            <span class="leading-5 text-sm font-light">International Islamic University Malaysia</span>
            <span class="leading-5 text-xs font-light">Jun, 2014 - Oct, 2017</span>
            <p class="leading-5 text-sm text-justify">Designed SRAM circuit that can prevent Single Event Upset (SEU) by radiation.</p>
          </div>
        </li>
      </ol>
    </div>
    <div class="flex flex-col justify-start items-start px-4 pt-4 mb-2">
      <span class="text-xl font-bold primary-text mb-2">Work Experience</span>
      <ol class="relative border-l border-gray-200 space-y-2">
        <li class="ml-4">
          <div class="absolute w-3 h-3 primary-bg rounded-full mt-1 -left-1.5 border border-white"></div>
          <div class="flex justify-between items-center mb-2 primary-text text-sm">
            <span class="leading-5">Oct, 2020 - Present</span>
            <div class="flex items-center">
              <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
              <span>Kuala Lumpur</span>
            </div>
          </div>
          <div class="flex flex-col">
            <span class="leading-5 text-base font-bold">IT Helpdesk</span>
            <span class="leading-5 text-base font-semibold">Huawei Technologies (Malaysia) Sdn Bhd</span>
            <ul class="list-disc pl-4 text-sm text-justify">
              <li>
                Handled service requests of Huawei's clients from the South Pacific Region using iCare system/emails/calls, ensured the smooth progress of service, followed up on pending requests, analyzed service data, and provided reports to internal teams and customers.
              </li>
              <li>
                Managed calls and tickets on the iCare system, communicated with customers to provide updates, conducted surveys and test calls, and maintained proficiency in maintenance processes.
              </li>
            </ul>
          </div>
        </li>
        <li class="ml-4">
          <div class="absolute w-3 h-3 primary-bg rounded-full mt-1 -left-1.5 border border-white"></div>
          <div class="flex justify-between items-center mb-2 primary-text text-sm">
            <span class="leading-5">Jan, 2018 - Dec, 2019</span>
            <div class="flex items-center">
              <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
              <span>Johor</span>
            </div>
          </div>
          <div class="flex flex-col">
            <span class="leading-5 text-base font-bold">Quality Control Engineer</span>
            <span class="leading-5 text-base font-semibold">Japan Solderless Terminal (JST) Connector (M) Sdn. Bhd</span>
            <ul class="list-disc pl-4 text-sm text-justify">
              <li>
                Ensured products quality and standard requirements were met, managed to reduce product recall.
              </li>
              <li>
                Improved company methods for product inspection by increasing QC sampling frequency, managed to detect product errors quickly and reduce amount of losses.
              </li>
            </ul>
          </div>
        </li>
        <li class="ml-4">
          <div class="absolute w-3 h-3 primary-bg rounded-full mt-1 -left-1.5 border border-white"></div>
          <div class="flex justify-between items-center mb-2 primary-text text-sm">
            <span class="leading-5">July, 2016 - Dec, 2017</span>
            <div class="flex items-center">
              <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
              <span>Johor</span>
            </div>
          </div>
          <div class="flex flex-col">
            <span class="leading-5 text-base font-bold">Technical Support Executive</span>
            <span class="leading-5 text-base font-semibold">AEGIS BPO Sdn. Bhd</span>
            <ul class="list-disc pl-4 text-sm text-justify">
              <li>
                Research and identify solutions to technical issues and managed to solved more than 30% of all issues per day.
              </li>
              <li>
                Liaised with customers to get to the root of the problems they faced, able to fix customers' issues quickly and efficiently within required time limits.
              </li>
            </ul>
          </div>
        </li>
        <li class="ml-4">
          <div class="absolute w-3 h-3 primary-bg rounded-full mt-1 -left-1.5 border border-white"></div>
          <div class="flex justify-between items-center mb-2 primary-text text-sm">
            <span class="leading-5">Feb, 2015 - June 2016</span>
            <div class="flex items-center">
              <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
              <span>Kuala Lumpur</span>
            </div>
          </div>
          <div class="flex flex-col">
            <span class="leading-5 text-base font-bold">Admin</span>
            <span class="leading-5 text-base font-semibold">IIUM Research Management Center</span>
            <ul class="list-disc pl-4 text-sm text-justify">
              <li>
                Carried out administrative duties and maintained the research center filing system.
              </li>
              <li>
                Assisted research center visitors and provided their needs.
              </li>
            </ul>
          </div>
        </li>
      </ol>
    </div>
  </div>
</template>

<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Mulish:ital,wght@0,200;0,300;0,400;0,600;1,700;1,900&display=swap');

.page {
  @apply mt-6 w-[7.25in] h-[10.75in] m-auto;
  page-break-after: always !important;
}

.primary-bg {
  @apply text-white bg-[#4BB2DB];
}

.primary-text {
  @apply text-[#4BB2DB];
}

.show-page-border {
  @apply border-2 border-gray-600;
}

.profile-pic {
  background-image: url('./assets/passport.jpg');
  background-position: top;
  background-size: 110px;
  background-repeat: no-repeat;
  background-color: white;
}
</style>